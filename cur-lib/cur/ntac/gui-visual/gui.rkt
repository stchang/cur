...
(define (get-panel-content-for-ntt-ext this-ntt-ext parent new-ntt-ext-callback undo-history-getter)
  ...
  (when (is-tagged-ntt? focus)
    (define tag-panel (new group-box-panel%
                           (parent panel)
                           (label "Generated by tactic")))
    (define tag-num (ntt-get-tag focus))
    (define undo-history (undo-history-getter))
    (define tag-message
      (cond [(= tag-num -1) "(Passed into tactic)"]
            [(or (< tag-num -1) (>= tag-num (length undo-history))) "(Invalid tag, don't use display-focus-tree more than once in a proof script)"]
            [else (interaction-history-tactic-str (list-ref undo-history tag-num))]))
          
    (new pretty-message% (parent tag-panel) (label tag-message)))
...
(define current-nttz-info-panel%
  (class single-ntt-ext-child-panel%
    (init-field new-ntt-ext-callback undo-history-getter)

    (super-new)
    
    (define/augment (new-ntt-ext ntt-ext)
      (get-panel-content-for-ntt-ext ntt-ext this new-ntt-ext-callback undo-history-getter))))
...
(define interaction-panel%
  (class vertical-panel%
                           ...
                           (main-thread-runner
                            (λ ()
                              (with-handlers ([exn:fail? (λ (e) (displayln (exn->string e)) current-nttz)])
                                ...
                                (tag-untagged-nttz-with nttz-untagged (length undo-buffer)))))) ; Tag with position in undo/redo list (0-indexed)
    ...
    ; So that the first item is at index 0
    (define/public (get-undo-history)
      (reverse undo-buffer))
...
(define (test-frame nttz preload-interaction-history)
  ...
  (parameterize ([current-eventspace es])
    ...
    ; Info panel will query the undo history before the interactions panel is initialized
    (define hacky-undo-history-getter (λ () '()))
    ...
    (define info-panel (new current-nttz-info-panel%
                            ...
                            [undo-history-getter (λ () (hacky-undo-history-getter))]))
    ...
    (set! hacky-undo-history-getter (λ () (send interactions-panel get-undo-history)))
